...
class draw_polyline:
    ...
    def start_draw_line(self, event):
        ...
        if self.click_num == 0:
            ...
            if self.vertical:
                self.lines_list.append(self.canvas.create_line(
                    coords[2], coords[3], coords[2], y+5))
            else:
                self.lines_list.append(self.canvas.create_line(
                    coords[2], coords[3], x+5, coords[3]))
    ...
    def start_draw_line_1(self, event):
        ...
        if self.click_num == 0:
            if self.vertical:
                self.lines_list.append(self.canvas.create_line(
                    x1, y1, x1, y+5, arrow='last', arrowshape=(8, 15, 3)))
            else:
                self.lines_list.append(self.canvas.create_line(
                    x1, y1, x+5, y1, arrow='last', arrowshape=(8, 15, 3)))
        ...
        if self.vertical:
            if abs(y-y0) > 20 or len(self.lines_list) == 1:
                ...
                if abs(x-x0) > 20:
                    ...
                    self.lines_list.append(self.canvas.create_line(
                        coords[2], coords[3], x, coords[3], arrow='last', arrowshape=(8, 15, 3)))
            elif abs(y-y0) < 20:
                self.canvas.delete(l)
                self.lines_list.pop(-1)
                self.canvas.itemconfig(
                    self.lines_list[-1], arrow='last', arrowshape=(8, 15, 3))
                # print('delete')
                self.vertical = not self.vertical
            ...
            if abs(x-x0) > 20 or len(self.lines_list) == 1:
                ...
                if abs(y-y0) > 20:
                    ...
                    self.canvas.itemconfig(l, arrow='')
                    self.lines_list.append(self.canvas.create_line(
                        coords[2], coords[3], coords[2], y, arrow='last', arrowshape=(8, 15, 3)))
