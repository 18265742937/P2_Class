...
class EnemyRabbit(pygame.sprite.Sprite):
    def __init__(self):
        ...
        # 添加敌人的射击状态
        shooting_state = EnemyRabbitShooting(self)
    ...
    def update(self, time_passed):
        ...
        if self.hp < 0:
            ...
            score += 1
            print("当前战绩:",score)
            corner = [0,0],[0,300],[500,0],[300,500]
            c = random.choice(corner)
            self.location = Vector2(c[0], c[1])
            self.hp = 100
            self.firepower += 1
...
class Light(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        if self.rect.x >= 640 or self.rect.x < 0 or self.rect.y < 0 or self.rect.y >= 480:
            self.kill()

class EnemyRabbitShooting(State):
    def __init__(self, enemyRabbit):
        State.__init__(self, "Shooting")
        self.enemyRabbit = enemyRabbit
        self.finish = enemyRabbit.firepower
    ...
    def do_actions(self):
        ...
        if random.randint(0,50) == 1:
            ...
            arrow = Arrow(enemyRabbit, (rabbit.rect.x, rabbit.rect.y))
            group_Enemy_arrow.add(arrow)
            angle = math.atan2(rabbit.rect.y-enemyRabbit.rect.y,rabbit.rect.x - enemyRabbit.rect.x) 
            self.enemyRabbit.shadow = pygame.transform.rotate(self.enemyRabbit.image, 360-angle*57.29)
    ...
    def entry_actions(self):
        print("进入射击状态")
        self.finish = self.enemyRabbit.firepower
...
class Arrow(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        if self.rect.x <= 0 or self.rect.x >= 640 or self.rect.y <= 0 or self.rect.y >= 480:
            self.kill() 
...
def checkCollision():
    ...
    if collide_barrier_enemy:
        for enemy in list(collide_barrier_enemy):
            enemy.destination *= -1

    collide_barrier_light = pygame.sprite.groupcollide(group_barrier, group_light, False, True)
    collide_rabbit_light = pygame.sprite.groupcollide(group_rabbit,group_light,False, True)
    if collide_rabbit_light:
        print("糟糕，被发现了")
        enemyRabbit.brain.set_state("Shooting") 

    collide_rabbit_enemyArrow = pygame.sprite.groupcollide(group_rabbit, group_Enemy_arrow,False, False) 
    if collide_rabbit_enemyArrow:
        rabbit.hp -= 1
    collide_barrier_enemyArrow = pygame.sprite.groupcollide(group_barrier, group_Enemy_arrow,False, True) 
...
if __name__ == "__main__":
    ...
    group_Enemy_arrow = pygame.sprite.Group()
    ...
    while True:
        ...
        group_arrow.update()
        enemyBeKill = group_enemy.update(time_passed/8)
            
        group_barrier.update()
        group_light.update()
        group_Enemy_arrow.update()
