...
def main():
    ...
    record_button = Button(screen, 380, 20, 50)
    record_button.text = "record"
    record_button.color = (100,100,100)

    back_button = Button(screen, 380, 600, 50)
    back_button.text = "back"
    back_button.color = (100,100,100)

    record_fonts = pygame.font.Font(None,32)

    with open("record.txt","r",encoding="utf-8") as f:
        record_text = f.read()
    record_text = record_text.split("\n")
    r = []
    for x in record_text:
        print(r)
        r.append(int(x[2:])) 
    top_score = r[0]
    ...
    while True:
        ...
        if record_show:
            ...
            for event in pygame.event.get(): 
                position = pygame.mouse.get_pos()
                isPressed = pygame.mouse.get_pressed()[0]
                if back_button.check_click(position) and isPressed:
                    record_show = 0
                if event.type == locals.QUIT:
                    pygame.quit()
                    sys.exit()

            screen.blit(font.render("记录" , True,(200, 100, 100)),(100, 100))
            screen.blit(font.render(record_text[0] , True,(100, 200, 100)),(100, 150))
            screen.blit(font.render(record_text[1] , True,(100, 180, 100)),(100, 200))
            screen.blit(font.render(record_text[2] , True,(100, 160, 100)),(100, 250))
            screen.blit(font.render(record_text[3] , True,(100, 140, 100)),(100, 300))
            screen.blit(font.render(record_text[4] , True,(100, 120, 100)),(100, 350))
        ...
        if isStart == 0:
            ...
            for event in pygame.event.get():
                ...
                if record_button.check_click(position) and isPressed:
                    record_show = 1
        ...
        if alive == 1:
            ...
            for event in pygame.event.get():
                if event.type == locals.QUIT:
                    ...
                    r.append(player.score)
                    r.sort(reverse = True)
                    r = r[:5]
                    score_text = "1、%d\n2、%d\n3、%d\n4、%d\n5、%d"%(r[0], r[1], r[2], r[3], r[4])
            ...
            if player.score > top_score:
                screen.blit(font.render("新纪录！" , True,(255, 0, 0)),(x_font+30, y_font+50))
