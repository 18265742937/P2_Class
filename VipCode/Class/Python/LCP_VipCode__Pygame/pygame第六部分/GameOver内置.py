...
class EnemySprite(pygame.sprite.Sprite):  # 敌方战机
    ...
    def fire(self):
        if self.bullettype == 0:
            ...
            if self.timer == 100:
                self.timer = 0
                bullet = EnemyBulletSprite("images/bullet/enemy_bullet1.png", [self.rect.x + 40, self.rect.y + 39.5], 5,
                                           0)
                # 添加左右偏移的敌机子弹
                bullet1 = EnemyBulletSprite("images/bullet/enemy_bullet1.png", [self.rect.x + 40, self.rect.y + 39.5],
                                            5, -1)
            ...
            if self.timer > 100:
                bullet = EnemyBulletSprite("images/bullet/boss_bullet2.png", [self.rect.x + 120, self.rect.y + 39.5], 2,
                                           -1)
                bullet2 = EnemyBulletSprite("images/bullet/boss_bullet2.png", [self.rect.x + 60, self.rect.y + 39.5], 2,
                                            1)
                self.bulletgroup.add(bullet, bullet2)
                self.timer = 0
...
def main():
    ...
    while True:
        ...
        if collide_player_enemy:
            # 被碰撞的敌机对象调用爆炸方法：
            values_enemy = list(collide_player_enemy.values())[0]
            values_enemy[0].bomb()
            print(list(collide_player_enemy.values()))

            values_enemy[0].hp = 0
            player.hp -= values_enemy[0].hp
            player.score += values_enemy[0].hp
        ...
        if collide_player_BOSS:
            values_boss = list(collide_player_BOSS.values())[0][0]
            values_boss.hp == 0
            values_boss.bomb()
            player.hp -= 10
            player.score += 10
        ...
        if collide_playerBullet_enemy:
            values_enemy = list(collide_playerBullet_enemy.values())[0][0]
            values_enemy.hp -= 1
            if values_enemy.hp <= 0:
                values_enemy.bomb()
                player.score += 2
        ...
        if collide_playerBullet_BOSS:
            values_boss = list(collide_playerBullet_BOSS.values())[0][0]
            values_boss.hp -= 1
            if values_boss.hp <= 0:
                values_boss.bomb()
                player.score += 10
        ...
        score = font.render("Score:" + str(player.score), True, "red")
        screen.blit( score, (20,20))
        hp = font.render("HP:" + str(player.hp), True, "red")
        screen.blit( hp, (20, 60))
