...
class Snake():
    ...
    def addButton(self, txt, pos, ic, ac):
        ...
        smallText = pygame.font.Font('STXINGKA.TTF', 20)
    ...
    #         f.write(str(self.score) + "\n")
        f = open("top5.conf", "a", encoding="utf8")
        f.write(str(self.score) + "\n")
        f.close()

    def rscore(self):
        """读取文件"""
        self.writeFont("STXINGKA.TTF", 72, "排行榜", "red", 220, 0)
        for x in range(5):
            self.writeFont("stxingka.ttf", 45, "第%s名:0分" % (x + 1), "orange", 180, 100 + x * 55)
