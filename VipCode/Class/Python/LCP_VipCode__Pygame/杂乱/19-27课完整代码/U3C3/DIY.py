...
class Pipe():
    def __init__(self, image, top,xx):
        self.image = image
...
class APP():
    ...
    def main(self):
        ...
        pipe_up = Pipe(self.pipe_up_image,260 + 80 + y ,self.width)
        pipe_down = Pipe(self.pipe_down_image,-60 -80 + y, self.width)



        y2 = random.randint(-120, 120)
        pipe_up2 = Pipe(self.pipe_up_image,260 + 80 + y2, self.width+self.width/2)
        pipe_down2 = Pipe(self.pipe_down_image, -60 - 80 + y2,self.width+self.width/2)
        ...
        while True:
            ...
            pipe_up2.update()  #-----------------
            pipe_down2.update()  #-----------------
            self.screen.blit(pipe_up2.image, pipe_up2.rect)
            self.screen.blit(pipe_down2.image, pipe_down2.rect)
